...
class InterFrameAligner : public rclcpp::Node {
 public:
  explicit InterFrameAligner(const rclcpp::NodeOptions &options)
    ...
    rclcpp::QoS qos(10); // 稍微给点缓存队列，防止高频点云丢失
    qos.durability(RMW_QOS_POLICY_DURABILITY_VOLATILE);        // 匹配 Fast-LIO 和 pcl_ros
    qos.reliability(RMW_QOS_POLICY_RELIABILITY_BEST_EFFORT);   // 尽力而为，最不容易出现不兼容
  ...
  void publishTF() {
    ...
    // 让 map->odom 之间的 TF 关系
    // 在等待点云配准的间隙，TF 时间戳可以连续不断
