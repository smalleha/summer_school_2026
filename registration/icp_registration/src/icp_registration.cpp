...
IcpNode::IcpNode(const rclcpp::NodeOptions &options)
  ...
  // 设置粗配准源点云
  icp_rough_.setMaximumIterations(rough_iter_);    // 迭代次数
  icp_rough_.setInputTarget(rough_map_);

  // 设置细配准源点云
  icp_refine_.setMaximumIterations(refine_iter_);   // 迭代次数
  ...
  // xy,yaw 分辨率
  xy_resolution_ = this->declare_parameter("xy_resolution", 0.2);
  yaw_resolution_ =
...
Eigen::Matrix4d IcpNode::multiAlignSync(PointCloudXYZI::Ptr source,
  ...
  for (int i = -1; i <= 1; i++) {
    for (int j = -1; j <= 1; j++) {
      for (int k = -18; k <= 18; k++) {
        ...
        Eigen::Vector3f pos(xyz(0) + i * xy_resolution_, xyz(1) + j * xy_resolution_,
                            xyz(2));
  ...
  // 遍历每一个初始位姿，进行粗配准，并提取误差分数，选择误差分数最小的位姿作为细配准的初始位姿
